...
import {
  ...
  type CashierVendorWithdrawalCompletedRow,
  type CashierVendorWithdrawalRejectedRow,
  type CashierVendorWithdrawalRow,
...
function RejectModal({
  ...
  const handleReject = async () => {
    ...
    const res = await vendorWithdrawalService.rejectRequest(
      requestId,
      ...
      <div className="w-full max-w-sm rounded-2xl border bg-white p-6 shadow-xl">
        ...
        <div className="mt-5 flex flex-col gap-4">
          <p className="text-sm text-muted-foreground">
            ...
            's remittance of{' '}
            <span className="font-semibold text-foreground">{peso(amount)}</span>.
          </p>
...
function PendingTable({ search, onRefresh }: PendingTableProps) {
  const [data, setData] = useState<CashierVendorWithdrawalRow[]>([]);
  const [total, setTotal] = useState(0);
  const [totalPages, setTotalPages] = useState(1);
  const [page, setPage] = useState(1);
  const [loading, setLoading] = useState(false);
  const [actionLoading, setActionLoading] = useState<string | null>(null);
  const [rejectTarget, setRejectTarget] = useState<CashierVendorWithdrawalRow | null>(null);

  const load = useCallback(
    async (p: number) => {
      setLoading(true);
      try {
        const res = await vendorWithdrawalService.listPendingRequests(p, search);
        setData(res.data);
        setTotal(res.total);
  ...
  const handleComplete = async (row: CashierVendorWithdrawalRow) => {
    setActionLoading(row.id);
    const res = await vendorWithdrawalService.completeRequest(row.id);
    if (res.success) {
      load(page);
      onRefresh();
    } else {
      alert(res.error?.message ?? 'Failed to complete request.');
    }
    ...
    <>
      ...
      {rejectTarget && (
        <RejectModal
          ...
          vendorName={rejectTarget.full_name}
          stallName={rejectTarget.stall_name}
...
function CompletedTable({ search }: { search: string }) {
  const [data, setData] = useState<CashierVendorWithdrawalCompletedRow[]>([]);
  const [total, setTotal] = useState(0);
  const [totalPages, setTotalPages] = useState(1);
  const [page, setPage] = useState(1);
  const [loading, setLoading] = useState(false);
  ...
  const load = useCallback(
    async (p: number) => {
      setLoading(true);
      try {
        const res = await vendorWithdrawalService.listCompletedRequests(p, search);
        setData(res.data);
        setTotal(res.total);
        setTotalPages(res.total_pages);
      } catch {
          ...
          {loading ? (
            ...
            data.map((row) => (
              <TableRow key={row.id} className="hover:bg-muted/40">
                ...
                <TableCell className="px-5 py-4 text-sm text-muted-foreground">
                  {row.stall_name || <span className="italic text-xs">—</span>}
                </TableCell>
                <TableCell className="px-5 py-4 text-sm font-semibold text-red-600">
                  {peso(row.amount)}
                </TableCell>
...
function RejectedTable({ search }: { search: string }) {
  const [data, setData] = useState<CashierVendorWithdrawalRejectedRow[]>([]);
  const [total, setTotal] = useState(0);
  const [totalPages, setTotalPages] = useState(1);
  const [page, setPage] = useState(1);
  const [loading, setLoading] = useState(false);
  ...
  const load = useCallback(
    async (p: number) => {
      setLoading(true);
      try {
        const res = await vendorWithdrawalService.listRejectedRequests(p, search);
        setData(res.data);
        setTotal(res.total);
        setTotalPages(res.total_pages);
      } catch {
          ...
          {loading ? (
            ...
            data.map((row) => (
              <TableRow key={row.id} className="hover:bg-muted/40">
                ...
                <TableCell className="px-5 py-4 text-sm font-semibold text-muted-foreground">
                  {peso(row.amount)}
                </TableCell>
                <TableCell className="px-5 py-4 text-sm text-muted-foreground">
                  {REJECTION_REASON_LABELS[
...
export const CashierVendorRemitPage: React.FC = () => {
  const [tab, setTab] = useState<TabType>('pending');
  const [search, setSearch] = useState('');
  const [pendingCount, setPendingCount] = useState(0);

  const refreshCount = useCallback(async () => {
    const count = await vendorWithdrawalService.getPendingCount();
    ...
    <div className="w-full px-1">
      <main className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div>
            <h1 className="text-2xl font-semibold tracking-tight">Vendor Remittance</h1>
            <p className="mt-1 text-sm text-muted-foreground">
              Review and process remittance requests from vendors.
            </p>
          </div>
        </div>
        ...
        <div className="flex flex-col gap-0">
          ...
          <div className="rounded-b-2xl rounded-tr-2xl border bg-white overflow-hidden">
            ...
            <div className="border-b px-6 py-5">
              <div className="relative w-full max-w-sm">
                ...
                <Input
                  ...
                  placeholder="Search by vendor name or stall..."
            ...
            <div className="p-6">
              {tab === 'pending' && (
                <PendingTable search={search} onRefresh={refreshCount} />
              )}

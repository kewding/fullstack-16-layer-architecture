...
import { topUpRequestService } from '../pages/notification-page/services/notification.service';
...
function useUserUnreadCount() {
  ...
  useEffect(() => {
    ...
    const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
    const ws = new WebSocket(`${protocol}//${window.location.host}/api/customer/notifications/ws`);
        ...
        <SidebarGroup>
          ...
          <SidebarMenu className="flex flex-col gap-1">
            ...
            {UserSidebarSections.navMain.map((item) => {
              ...
              const isNotifItem = item.url === '/user/notifications';
                  ...
                  >
                    <NavLink to={item.url} className="flex items-center gap-2 w-full">
                      <div className="relative shrink-0">
                        ...
                        {isNotifItem && unreadCount > 0 && isCollapsed && (
